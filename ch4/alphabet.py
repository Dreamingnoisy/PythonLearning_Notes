...
#Codes above here are from the author.


def draw_prime(t,size):
    bob.pu()
    bob.bk(size/2)
    bob.lt(90)
    bob.fd(size*3)
...
if __name__ == '__main__':
    ...
    bob.bk(300)
    bob.pd()
    for f in [draw_h, draw_a, draw_p, draw_p, draw_y]:
        f(bob, size)
        skip(bob, size)
    skip(bob, size)
    for f in [draw_f, draw_a, draw_t, draw_h, draw_e,draw_r]:
        f(bob, size)
        skip(bob, size)
